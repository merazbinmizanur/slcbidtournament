...
// FIX: Player Schedule এ শুধু গ্রুপ ম্যাচ দেখাবে
const publicMatches = state.matches.filter(m =>
    m.isPublic && (!m.round || m.round.toLowerCase().includes('group'))
);
...
// শুধু ম্যানেজারের নিজের টিমের ম্যাচগুলো এবং যেগুলো পাবলিক করা হয়েছে (শুধুমাত্র গ্রুপ স্টেজ)
const myMatches = state.matches.filter(m =>
    (m.team1Id === u.id || m.team2Id === u.id) &&
    m.isPublic &&
    (!m.round || m.round.toLowerCase().includes('group'))
);
...
// FIX: শুধুমাত্র গ্রুপ স্টেজের ম্যাচগুলো লিস্টে দেখাবে, প্লেঅফ নয়
const groupMatches = state.matches.filter(m => !m.round || m.round.toLowerCase().includes('group'));
...
if (m.status === 'pending_lineup') {
    ...
    // FIX: Added safe checks to prevent crashes on older matches
...
const texts = {
    ...
    sRule4Desc: isBn ? "একটি ম্যাচে প্রতিটি দল সর্বোচ্চ ১টি সাবস্টিটিউট (Sub) এবং ১টি সোয়াপ (Swap) ব্যবহার করতে পারবে। সোয়াপ করার ক্ষেত্রে প্রতিপক্ষের আগাম অনুমতির কোনো প্রয়োজন নেই। SUB/SWAP করতে হলে অবশ্যই ডেডলাইনের অন্তত ৩০ মিনিট আগে করতে হবে। ১ মিনিট পরে ( উদা ১২:০১) হলেও সেই সাব গ্রহণযোগ্য হবে না।" : "Each team is allowed a maximum of 1 Substitute and 1 Swap per match. No prior permission from the opponent is required to execute a swap. Substitutions / SWAP must be made at least 30 minutes before the deadline. Any request made even 1 minute late(eg 12:01) will not be accepted.",
...
async function forceAutoLineup(matchId) {
        askConfirm('Are you sure you want to FORCE Auto-Lineup? This will automatically pick available players (excluding suspended) and set the Manager as Captain.', async () => {
                        ...
                        if (!m.team1Id || !m.team2Id) {
                            return notify('Cannot Auto Fill! Both teams must be decided first.', 'alert-circle');
                        }
